          ...
          <div className="space-y-1">
            ...
            {isGuest && (
              <p className="text-xs font-bold text-amber-600 dark:text-amber-400">
                Guest preview uses static data. Create, edit, delete, duplicate,
                and PDF actions are disabled.
              </p>
            )}
          </div>
            ...
            <table className="w-full border-collapse min-w-[900px]">
              <thead>
                <tr className="border-b border-slate-50 dark:border-slate-800/50 h-16 text-left">
                  <th className="px-8 text-[10px] font-black text-slate-400 uppercase tracking-widest">
                    GR Details
                  </th>
                  <th className="px-8 text-[10px] font-black text-slate-400 uppercase tracking-widest">
                    Consignor → Consignee
                  ...
                  <th className="px-8 text-[10px] font-black text-slate-400 uppercase tracking-widest text-right">
                    Actions
                  </th>
                </tr>
              </thead>
              <tbody className="divide-y divide-slate-50 dark:divide-slate-800/50">
                {isLoading ? (
                  ...
                  <tr>
                    <td colSpan={7} className="px-8 py-20 text-center">
                      <span className="text-sm font-medium text-red-400">
                        Failed to load GRs. Please try again.
                      </span>
                    </td>
                  </tr>
                ) : grs.length === 0 ? (
                  ...
                  grs.map((row) => {
                    const statusCfg = STATUS_MAP[row.status] ?? {
                      label: row.status,
                      className: "bg-slate-100 text-slate-600 border-slate-200",
                    };
                    ...
                    return (
                      ...
                      >
                        ...
                        <td className="px-8 py-5 whitespace-nowrap">
                          ...
                          {row.customer && (
                            <p className="text-[10px] text-sky-600 dark:text-sky-400 font-bold mt-1">
                        ...
                        <td className="px-8 py-5 whitespace-nowrap">
                          <span
                            className={cn(
                              "inline-flex items-center px-3 py-1 rounded-full border text-[10px] font-black uppercase tracking-widest whitespace-nowrap",
                              statusCfg.className,
                            )}
                          >
                            {statusCfg.label}
                        ...
                        <td className="px-8 py-5 text-right whitespace-nowrap">
                          <div className="flex items-center justify-end gap-1">
                            ...
                            >
                              {duplicateGR.isPending &&
                              duplicateGR.variables === row.id ? (
                                <Loader2 size={16} className="animate-spin" />
                              ) : (
                                <Copy size={16} />
                              )}
