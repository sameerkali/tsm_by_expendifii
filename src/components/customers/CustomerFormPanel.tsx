    ...
    <div ref={containerRef} className="relative">
      <div className="relative">
        <input
          ...
          placeholder={
            disabled ? "Enter pincode first" : "Select Locality / Area"
          }
          disabled={disabled}
          autoComplete="off"
          className={cn(
...
export function CustomerFormPanel({
  ...
  const onAutofill = useCallback(
    (payload: { city: string; district: string; state: string }) => {
      setForm((prev) => ({
        ...prev,
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    const onError = (error: unknown) => {
      ...
      if (Object.keys(mapped).length > 0) {
        ...
        setTouched((prev) => ({
          ...prev,
          ...Object.keys(mapped).reduce<Record<string, boolean>>(
            (acc, field) => {
              acc[field] = true;
              return acc;
            },
        ...
        >
          ...
          <Field label="Address" error={err("address")}>
            <textarea
              ...
              className={cn(
                inputClass,
                "resize-none h-auto py-3",
                err("address") && errorInputClass,
              )}
            />
          </Field>
              ...
              <div className="grid grid-cols-3 gap-4">
                ...
                <Field label="State" error={fieldErrors.state}>
                  <input
                    ...
                    className={cn(
                      inputClass,
                      fieldErrors.state && errorInputClass,
                    )}
                  />
                </Field>
              </div>
          ...
          {/* Pricing Section */}
          <Field label="Pricing Type & Default Rate" error={err("defaultRate")}>
              ...
              <div className="relative w-36 shrink-0">
                ...
                <input
                  ...
                  className={cn(
                    inputClass,
                    "pl-8",
                    err("defaultRate") && errorInputClass,
                  )}
                />
              </div>
            </div>
...
function CharCount({ value, max }: { value: string; max: number }) {
  ...
  return (
    <p
      className={cn(
        "text-[10px] font-medium text-right mt-0.5",
        isNear ? "text-amber-500" : "text-slate-400",
      )}
    >
      {len}/{max}
    </p>
